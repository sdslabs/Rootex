...
{
	return floor(intensity * NUMBER_OF_SHADES) / NUMBER_OF_SHADES;
}
...
float4 GetCelColorFromPointLight(PointLightInfo pointLight, float3 toEye, float3 normal, float4 worldPosition, float4 materialColor, float3 specularColor, float specPow, float specularIntensity, float isLit)
{
	...
	float3 reflected = floor(reflect(-normalizedRelative, normal) * NUMBER_OF_SHADES) / NUMBER_OF_SHADES;
    ...
    : SV_TARGET
{
	...
	float shade = floor(intensity * float(NUMBER_OF_SHADES));
	float brightnessOfShade = shade / float(NUMBER_OF_SHADES);
