...
Rml::Core::Character InputInterface::GetCharacterCode(Rml::Core::Input::KeyIdentifier key_identifier, int key_modifier_state)
{
...
void InputInterface::ProcessWindowsEvent(UINT message, WPARAM w_param, LPARAM l_param)
{
	...
	case WM_MOUSEMOVE:
		s_Context->ProcessMouseMove(LOWORD(l_param), HIWORD(l_param), GetKeyModifierState());
		break;

	case WM_MOUSEWHEEL:
		s_Context->ProcessMouseWheel(static_cast<float>((short)HIWORD(w_param)) / static_cast<float>(-WHEEL_DELTA), GetKeyModifierState());
		break;
	...
	case WM_KEYDOWN:
	{
		...
		s_Context->ProcessKeyDown(key_identifier, key_modifier_state);
	}
	break;
	...
	case WM_KEYUP:
		s_Context->ProcessKeyUp(key_identifier_map[w_param], GetKeyModifierState());
		break;
	}
}
