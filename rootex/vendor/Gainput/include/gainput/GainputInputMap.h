 ...
 * simultaneously or use different ones depending on game state. The user button IDs have to be unique per input mapBool.
 *
 * InputMap uses the provided InputManager to get devices inputs and process them into user-mapped inputs. After creating 
 * an InputMap, you should mapBool some device buttons to user buttons (using MapBool() or MapFloat()). User buttons are identified 
 ...
 * for any given InputMap. It's of course possible to mapBool multiple different device button to one user button.
 *
...
{
public:
	/// Initializes the mapBool.
	/**
	 * \param manager The input manager used to get device inputs.
	 * \param name The name for the input mapBool (optional). If a name is provided, it is copied to an internal buffer.
	...
	/// Unitializes the mapBool.
	~InputMap();

	/// Clears all mapped inputs.
	void Clear();

	/// Returns the input manager this input mapBool uses.
	const InputManager& GetManager() const { return manager_; }
	/// Returns the mapBool's name, if any.
	/**
	 * \return The mapBool's name or 0 if no name was set.
	 */
	const char* GetName() const { return name_; }
	/// Returns the mapBool's auto-generated ID (that should not be used outside of the library).
