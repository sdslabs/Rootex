...
gainput::InputMap mapBool(manager);
mapBool.MapBool(ButtonConfirm, mouseId, gainput::MouseButtonLeft);
...
while (game_running)
{
	...
	// Check button state
	if (mapBool.GetBoolWasDown(ButtonConfirm))
	{
		// Confirmed!
	}
}
...
On top of your gainput::InputManager, you should create at least one gainput::InputMap and mapBool some device-specific buttons to your custom user buttons using gainput::InputMap::MapBool() or gainput::InputMap::MapFloat(). This will allow you to more conventienly provide alternative input methods or enable the player to change button mappings.
