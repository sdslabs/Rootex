...
void SceneSettings::drawInputScheme(InputDescription& inputDesc)
{
	...
	if (device == 0)
	{
		...
		char name[20] = {};
		int nameSize = (int)InputManager::GetSingleton()->getMouse()->GetButtonName(inputDesc.button, name, 20) - 1;
		int currentKey = 0;
		for (int i = 0; i < ARRAYSIZE(keys); i++)
		{
			bool isEq = 1;
			int charIndex = 0;
			while (keys[i][charIndex])
			{
				isEq &= (keys[i][charIndex] == name[charIndex]);
				charIndex++;
			}
			isEq &= (charIndex == nameSize);
			currentKey = isEq ? i : currentKey;
		}
		ImGui::Combo("", &currentKey, keys, ARRAYSIZE(keys));
		inputDesc.button = InputManager::GetSingleton()->getMouse()->GetButtonByName(keys[currentKey]);
	}
	else if (device == 1)
	{
		char* keys[] = {
		...
		char name[20] = {};
		int nameSize = (int)InputManager::GetSingleton()->getKeyboard()->GetButtonName(inputDesc.button, name, 20) - 1;
		int currentKey = 0;
		for (int i = 0; i < ARRAYSIZE(keys); i++)
		{
			bool isEq = 1;
		...
		ImGui::Combo("", &currentKey, keys, ARRAYSIZE(keys));
		inputDesc.button = InputManager::GetSingleton()->getKeyboard()->GetButtonByName(keys[currentKey]);
	}
	else if (device == 1 || device == 2)
	{
		...
		char name[20] = {};
		int nameSize = (int)InputManager::GetSingleton()->getPad1()->GetButtonName(inputDesc.button, name, 20) - 1;
		int currentKey = 0;
		for (int i = 0; i < ARRAYSIZE(keys); i++)
		{
			bool isEq = 1;
		...
		ImGui::Combo("", &currentKey, keys, ARRAYSIZE(keys));
		inputDesc.button = InputManager::GetSingleton()->getPad1()->GetButtonByName(keys[currentKey]);
	}
	ImGui::SameLine();
	ImGui::SetNextItemWidth(ImGui::GetContentRegionAvailWidth() / 3.0f);
