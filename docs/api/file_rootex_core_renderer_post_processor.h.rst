
.. _file_rootex_core_renderer_post_processor.h:

File post_processor.h
=====================

.. contents:: Contents
   :local:
   :backlinks: none

Definition (``rootex\core\renderer\post_processor.h``)
------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_rootex_core_renderer_post_processor.h.rst





Includes
--------


- ``ASSAO.h``

- ``PostProcess.h``

- ``common.h`` (:ref:`file_rootex_common_common.h`)



Included By
-----------


- :ref:`file_rootex_framework_components_visual_camera_component.h`




Classes
-------


- :ref:`exhale_struct_struct_post_processing_details`

- :ref:`exhale_class_class_post_process`

- :ref:`exhale_class_class_post_processor`

